...
public class CodeGenVisitorTest implements ImageResources{
	...
	public void image7() throws Exception {
		...
		for (int y = 0; y < 1024; y++) {
			for (int x = 0; x < 1024; x++) {
				int pixelR = RuntimeFunctions.cart_x(RuntimeFunctions.polar_r(x, y),
						RuntimeFunctions.polar_a(x, y));
				int pixelA = RuntimeFunctions.cart_y(RuntimeFunctions.polar_r(x, y),
						RuntimeFunctions.polar_a(x, y));
				int pixel0 = ImageSupport.getPixel(loggedImage0, pixelR, pixelA);
	...
	//imageIO1: Testing failed with input: "imageIO1//args: <inputImageFullPath> <outputImageFullPath>\n image g;
	//\n file f = @ 1; \ng <- @ 0;\ng -> SCREEN;\ng -> f;\nimage h;\nh <- f; \nh -> SCREEN;". 
	@Test
	public void failedCheck1() throws Exception{
		devel = false;
		grade = true;
		String prog = "fail1";
		String input = prog 
				...
				;
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {imageFile1,"C:/Users/nutal/workspace/PLPHomework/src/ShelterPoint3.jpg"}; 
		runCode(prog, bytecode, commandLineArgs);		
...
//imageIO2//args: <imageURL>\n image g; \n file f = \"newImage.jpg\"; \ng <- @ 0;\ng -> SCREEN;\ng -> f;
	...
	@Test
	public void failedCheck2() throws Exception{
		devel = false;
		grade = true;
		String prog = "fail2";
		String input = prog 
				+ "\nimage g;"
				+ "\n file f = \"C:/Users/nutal/workspace/PLPHomework/src/ShelterPoint5.jpg\";"
				+ "\n g <- @ 0;\n"
				+ "g -> SCREEN;\n"
				+"g -> f;"
				+"\nimage h;"
	...
	//image10: Testing failed with input: "image10//args: <imageURL>\nimage[1024,1024] g;
	//\n\nimage[1024,1024] h; \ng <- @ 0;\ng -> SCREEN;\nh[[x,y]] =  g[x,Y-y];
	//h -> SCREEN; \n". expected:<0> but was:<9281988>
	@Test
	public void failedCheck3() throws Exception{
		devel = false;
		grade = true;
		String prog = "fail2";
		String input = prog 
		...
		for (int y = 0; y < 1024; y++) {
			for (int x = 0; x < 1024; x++) {
				int pixel1 = ImageSupport.getY(loggedImage0) - y;
				System.out.println(ImageSupport.getY(loggedImage0)-y);
				int pixelA = ImageSupport.getPixel(loggedImage1, x, y);
				int pixelB = ImageSupport.getPixel(loggedImage0, x, pixel1);
	...
	//image7: Testing failed with input: "image7//args: <inputImageURL> <outputImageURL>\nimage[1024,1024] g; 
		//\n\nimage[1024,1024] h; \ng <- @ 0;\n file f = @ 1; \ng -> SCREEN;\nh[[r,a]] =  g[r,a];h -> SCREEN; \nh -> f;
	@Test	
	public void failedCheck4() throws Exception{
		devel = false;
		grade = true;
		String prog = "fail2";
		String input = prog 
		...
		BufferedImage loggedImage0 = RuntimeLog.globalImageLog.get(0);
		BufferedImage loggedImage1 = RuntimeLog.globalImageLog.get(1);
		assertTrue(ImageSupport.compareImages(loggedImage1,loggedImage1));	
